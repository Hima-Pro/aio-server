...
for i in /var/www/src/setup/pre-init.d/*sh
do
	if [ -e "${i}" ]; then
...
if [ -d /var/lib/mysql/mysql ]; then
	...
	if [ "$MYSQL_ROOT_PASSWORD" = "" ]; then
		...
		echo $MYSQL_ROOT_PASSWORD > /var/www/src/setup/mysql-root-pw.txt
...
EOF
    ...
    # the /var/www/src/setup/initdb.d/ file is not empty
	if [ "$MYSQL_DATABASE" != "" ] && [ "$(ls -A /var/www/src/setup/initdb.d 2>/dev/null)" ]; then

		# start the server temporarily so that we can import seed files
        echo
        echo "Preparing to process the contents of /var/www/src/setup/initdb.d/"
        echo
		...
		for f in /var/www/src/setup/initdb.d/*; do
			case "$f" in
...
for i in /var/www/src/setup/pre-exec.d/*sh
do
	if [ -e "${i}" ]; then
